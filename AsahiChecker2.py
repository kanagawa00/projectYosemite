...
articles=[]
#f=open("AsahiCheck.txt","w",encoding="utf-8")
...
def newsdownload(browser,nl):
    ...
    for no,date,ttl,news,day,men,page,size in zip(nolist,datelist,ttllist,newslist,daylist,menlist,pagelist,lenlist):
        if news.text.startswith("※著作権などの関係で本文を表示できません。"):
            ...
            nl.append([no,date,ttl.text,day,men,page,size,news.text])
            #f.write("<H1>"+date+"</H1>"+"\n")
...
#早稲田検索ページを開ける
    ...
    time.sleep(2)
    browser.get("http://database.asahi.com/")
...
#データフレーム構築
df = pd.DataFrame(articles,columns=["No","時間","タイトル","朝夕刊","面","ページ","文字数","記事内容"])
#df.to_csv("AsahiCheck2.csv",encoding="utf-8",sep="\t", index=False)
df.to_excel('AsahiCheck.xlsx', sheet_name='sheet1')
df
